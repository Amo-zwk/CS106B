...
bool cmp(DataPoint a, DataPoint b) {
...
Vector<DataPoint> topK(istream& stream, int k) {
    Vector<DataPoint> sortArray;
    Vector<DataPoint> copyArray;
    DataPoint cur;
    while(stream >> cur) {
        sortArray.add(cur);
    }
    std::sort(sortArray.begin(),sortArray.end(),cmp); //从大到小排序
    if(sortArray.size() <= k) return sortArray;
    else {
        for(int i = 0; i < k; i++) {
            copyArray.add(sortArray[i]);
        }
        return copyArray;
    }
    return {};
